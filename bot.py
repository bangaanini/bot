...
# Mendapatkan Supabase URL dan API Key dari file .env
...
def kategori_handler(update: Update, context: CallbackContext) -> None:
    ...
    produk = supabase.table("products").select("*").eq("kategori", kategori_terpilih).execute().data
...
def layanan_handler(update: Update, context: CallbackContext) -> None:
    ...
    produk = supabase.table("products").select("*").eq("layanan", layanan_terpilih).execute().data
...
def paket_handler(update: Update, context: CallbackContext) -> None:
    ...
    produk = supabase.table("products").select("*").eq("layanan", layanan_terpilih).eq("paket", paket_terpilih).execute().data
...
def kembali_handler(update: Update, context: CallbackContext) -> None:
    ...
    if len(data) == 3:
        ...
        produk = supabase.table("products").select("*").eq("kategori", kategori).execute().data
...
def layanan_kembali_handler(update: Update, context: CallbackContext) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton("Order", callback_data=f"order:{layanan_terpilih}")],
        [InlineKeyboardButton("•Kembali•", callback_data="back:category:{context.user_data.get('kategori')}")]  # Kembali ke daftar kategori
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
